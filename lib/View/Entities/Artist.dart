...
import 'package:project_app/Model/artistModel.dart';
...
class _ArtistState extends State<Artist> {
  ...
  Future<void> _getReviews() async {
    final response =
        await Request.get('artistreviews/' + widget.slug.toString());
    print("getReviews");
    setState(() {
  ...
  void followArtist() async {
    dynamic response = await Request.put('follow',
        jsonEncode({"artist": artist.id, "follow": true, "type": "artist"}));
  }

  void unfollowArtist() async {
    dynamic response = await Request.put(
        'follow',
        jsonEncode(
            {"organizer": artist.id, "follow": false, "type": "artist"}));
  }
  ...
  double rating = 0;
  void onSharePress() {
    Share.share(artist.name + ' ' + Request().host + artist.slug.toString());
  }
  ...
  Future<void> getArtist() async {
    ...
    String response = await Request.get('artist/' + widget.slug.toString());

    setState(() {
      isLoading = false;
      languagePack = Translator.fromJson(jsonDecode(text));
      artist = ArtistModel.fromJson(jsonDecode(response));
